...
#Holds the moves array for a player
class kpb24_Player:
	def __init__(self):
		self.moves = []
	
def kpb24_AutoPlayer(board):
	board = kpb24_Board(np.array(board.config)) #use np array bc easier to deal with multidimensional array
	boards, moves = board.moves(kpb24_Player())
	kpb24_determineExplored().addToExplored(board) #get all of the child node information into the explored list so we can look through
									#to determine which move is best
	#body of alpha beta search
	#Use a random move if the board isn't in the explored list
	for i, b in enumerate(boards):
		move = moves[i]
 		if move in kpb24_determineExplored().exploredList:
			score = maxValue(boards, float("-inf"), float("inf")) #finds the max value and use as score to determine the bestscore
			if score > bestScore:
				bestScore = score
				bestAction = move
		else:
			bestAction = random.choice(moves)
	return bestAction
...
def maxValue(state, alpha, beta):
	...
	value = float("-inf")
	for s in kpb24_determineExplored().exploredList:
...
class kpb24_Board:
    def __init__(self, board):
        self.board = np.array(board) #use np because it's easier to work with a multidimensional array

	#Goes through the array of queen moves returned by moveQueenPosition and updates the board 
	#to reflect the new locations. Then, it goes through the array of arrow moves returned by
	#moveArrowPosition() and updates the board to reflect the new location.
    def moves(self, player):
        boards = []
        moves = []
        for qMove in self.moveQueenPosition():
            src, dst = qMove
            updatedBoard = self.copyBoard(self.board, src, dst, "queen") #the updated board for after queen moved
            for a in self.moveArrowPosition(updatedBoard, dst):
                moves.append((src, dst, a)) #add the arrow and where it's going to moves array
                boards.append(self.copyBoard(updatedBoard.board, a, a, "arrow")) #add the updated board
                
        return boards, moves 

	#Moves the position of the queen by using the helper function movePosition()
	#Goes through the board looking for white player pieces (Q) and sends their location
	#to movePosition() where a final source and destination is determined
    def moveQueenPosition(self):
        queenMoves = []
        for row in range(10):
            for column in range(10):
                if self.board[row, column] == 'Q':
                    queenMoves += self.movePosition((row, column))
                    
        return queenMoves      
        
	#Moves the position of the arrow by taking a board and the position(row, column)
	#it should move to as arguments. Uses movePosition() as a helper function.
    def moveArrowPosition(self, board, pos):
    
        return [move[1] for move in board.movePosition(pos)]
        
    #Helper for moving an arrow or queen on board by using directions (up, down, left, right, and diagonals)
    #Checks to make sure the position came up with is legal before appending to moves array
    def movePosition(self, loc):
        moves = []
		#The queen or arrow can move up and down, to each side, and diagonally
        for d in ["up", "upright", "down", "downleft", "right", "downright", "left", "upleft"]:
            directions = d
            illegalMove = False
            while not illegalMove:
            	row, column = loc
            	#change the row number and column number according to how many directions there are for each
            	row -= directions.count("up")
            	row += directions.count("down")
            	column += directions.count("left")
            	column -= directions.count("right")
            	
            	#make sure it's still in the boundaries of the board
            	if 0 < row < 10 and 0 <= column < 10:
            		pos, newLoc = self.board[row, column], (row, column) #it's on the board
            	else:
            		pos, newLoc = 0, (row, column) 
                if pos == '.':
                    moves.append((loc, newLoc)) #add to moves array if it's an empty spot
                    directions += d #if it's a legal move then add it to the string of directions
                else:
                    illegalMove = True #move can't be made
                    
        return moves
        
	#Makes a copy of the updated boards when a queen or arrow is moved
    @staticmethod
    def copyBoard(board, src, dst, moved):
    	if moved is "queen":
			copy = np.copy(board)
			copy[dst] = copy[src]
			copy[src] = '.'
...
class kpb24_determineExplored():
    def __init__(self):      
    	self.exploredList = dict()                            
        self.whitePlayer = kpb24_Player()
	
	#Expands to look at potential moves and returns the first unseen child it finds
	#in addition to returning the player's opponent
    def expandNode(self, state, player):
        moves, _ = state.moves(player)
        for child in moves:
            if child not in self.exploredList:
                return child, player
                
	#Adds the state to the players moves array if child is not in explored list
    def choose(self, state, player):
        childBoards, _ = state.moves(player)
        for c in childBoards:
            if c not in self.exploredList:
                return state, player
                
    def addToExplored(self, currentBoard):
        child, player = self.choose(currentBoard, self.whitePlayer)
        self.expandNode(child, player)
